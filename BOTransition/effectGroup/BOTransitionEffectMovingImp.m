...
- (void)bo_transitioning:(BOTransitioning *)transitioning
    ...
    switch (step) {
        case BOTransitionStepInstallElements: {
            [self.class installEffect:transitioning
                             elements:elements
                               config:self.configInfo ? : [self defaultConfigInfo]];
        }
            break;
        default:
            break;
...
+ (void)installEffect:(BOTransitioning *)transitioning
    ...
    NSDictionary *configinfo = config;
    ...
    BOOL hasBG = YES;
    NSNumber *bgval = [configinfo objectForKey:@"bg"];
    if (nil != bgval) {
